...
get.oc.obd<-function(targetT=0.3,targetE=0.2,ncohort=10,cohortsize=3,
                    startdose=3,
                     cutoff.eli.T=0.95,
                     N1=6, N2=9, cutoff.eli.E=0.95,u1,u2,ntrial=100,utilitytype = 1, randomtype = 1){
...
if(FALSE){
  ...
  if(targetT<0.05) {cat("Error: the target for toxicity is too low! \n"); return();}
  if(targetT>0.6)  {cat("Error: the target for toxicity is too high! \n"); return();}
  if(targetE<0.05) {cat("Error: the target for efficacy is too low! \n"); return();}
  if(targetE>0.8)  {cat("Error: the target for efficacy is too high! \n"); return();}
  if(u11<=0 | u11>=100) {cat("Error: the utility for (toxicity=1,efficacy=1) should lie inside (0,100)")}
  if(u00<=0 | u00>=100) {cat("Error: the utility for (toxicity=0,efficacy=0) should lie inside (0,100)")}
  if((targetT-p.saf)<(0.1*targetT)) {cat("Error: the probability deemed safe cannot be higher than or too close to the toxicity target! \n"); return();}
  if((p.tox-targetT)<(0.1*targetT)) {cat("Error: the probability deemed toxic cannot be lower than or too close to the toxicity target! \n"); return();}
  ...
  if(length(pT.true)!=length(pE.true)) {cat("Error: the length of toxicity probability vector does not match that of efficacy probability vector! \n"); return();}
  #if(length(pE)!=ndose) {cat("Error: the length of efficacy probability vector does not match the specified number of dose level! \n"); return();}
  if(rho<=-1 | rho>=1) {cat("Error: the correlation coefficient between toxicity and efficacy should lie inside (-1,1)! \n"); return();}
}
 ...
 set.seed(30); 
  for(trial in 1:ntrial){
    ...
    for(i in 1:ncohort){
      ...
      # obscohort = get.pts.outcome(d=d,dist=event.dist,coh.size=cohortsize,pE.true=pE.true,pT=pT.true,
                                  # r=rho,A=A,accrual.rate)

      # y01[d]<-y01[d]+sum(obscohort$tox==0 & obscohort$eff==1)
      # y10[d]<-y10[d]+sum(obscohort$tox==1 & obscohort$eff==0)
      # y11[d]<-y11[d]+sum(obscohort$tox==1 & obscohort$eff==1)
      # y00[d]<-y00[d]+sum(obscohort$tox==0 & obscohort$eff==0)
     ...
     # uu1<-rnorm(cohortsize,qnorm(pT.true[d]))
     #  uu2<-rnorm(cohortsize,qnorm(pE.true[d])+rho*(uu1-qnorm(pT.true[d])),sqrt(1-rho^2))
      # y01[d]<-y01[d]+sum(uu1<0&uu2>0)
      # y10[d]<-y10[d]+sum(uu1>0&uu2<0)
      # y11[d]<-y11[d]+sum(uu1>0&uu2>0)
      # y00[d]<-y00[d]+sum(uu1<0&uu2<0)
      n[d]<-n[d]+cohortsize
...
oc = get.oc.obd(targetT=0.3,targetE=0.2,ncohort=i,cohortsize=3,
                    startdose=3,
                     cutoff.eli.T=0.95,
                     N1=6, N2=9, cutoff.eli.E=0.9,u1,u2,ntrial=10000,utilitytype = utype, randomtype = rtype)
...
write.csv(outputmat,paste0(iii,"/boin12_random.csv"),row.names=FALSE)
