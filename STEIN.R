...
get.oc <- function(targetT,  targetE, psi1, psi2, ncohort, cohortsize, startdose=1, cutoff.eliT=0.95, cutoff.eliE=0.9, ntrial=10, utilitytype = 1, randomtype = 1)
{
target = targetT
cutoff.eli =  cutoff.eliT
...
acr=0;
exc=0;
temp=get.boundary(targetT, ncohort, cohortsize) 	
...
set.seed(30);
	...
	for(trial in 1:ntrial)
	{
		...
		{  			
			...
			if(n[d]>=3 && pbeta(targetE,yE[d]+1,n[d]-yE[d]+1)>cutoff.eliE) {elimiE[d]=1;}
...
oc = get.oc(targetT=TARGET.T,  targetE=TARGET.E, psi1, psi2, ncohort=i, cohortsize=3, startdose=STARTD, cutoff.eliT=0.95, cutoff.eliE=0.9, ntrial=10000, utilitytype = utype, randomtype = rtype)
